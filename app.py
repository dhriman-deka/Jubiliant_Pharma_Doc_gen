...
try:
    ...
    from app.utils.api import initialize_api, extract_document_content, analyze_template, get_preferred_model
except ImportError:
    # Fall back to the old module name
    from app.utils.gemini_api import initialize_gemini as initialize_api, extract_document_content, analyze_template, get_preferred_model
    ...
    if template_fields:
        ...
        with st.form("template_form"):
            ...
            if submit_button:
                ...
                # Add a navigation hint for tab selection
                st.session_state.go_to_export = True
        
        # Check if we should show a navigation button outside the form
        if st.session_state.get('go_to_export', False):
            st.info("You can now export your document in the Export Document tab.")
            # We cannot directly navigate tabs in Streamlit, so we guide the user
